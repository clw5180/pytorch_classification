...
albu_transforms_train =  [
                ...
                #                    border_mode=cv2.BORDER_REFLECT101, p=0.5),  # cv2.BORDER_REFLECT101, p=0.5),
                # # # border_mode=cv2.BORDER_REPLICATE  BORDER_REFLECT101 BORDER_CONSTANT
                # A.Transpose(p=0.5),
                # A.HorizontalFlip(p=0.5),
                # A.VerticalFlip(p=0.5),
                # A.RandomRotate90(p=0.5),  # A.OneOf([A.RandomRotate90(p=1), A.Transpose(p=1)], p=0.5),
                # A.HueSaturationValue(hue_shift_limit=0, sat_shift_limit=0.1, val_shift_limit=0.1, p=0.5),
