...
from utils.misc import AverageMeter, accuracy, accuracy_onehot
...
df = pd.read_csv(f"{ROOT_DIR}/2020.csv")
...
class CassavaLeafDataset(nn.Module):
    ...
    def __getitem__(self, index):
        ...
        soft_label = np.array([self.df.iloc[index, 3], self.df.iloc[index, 4], self.df.iloc[index, 5], self.df.iloc[index, 6],
                               self.df.iloc[index, 7]    ])
        label_new = 0.7*label+0.3*soft_label
...
def train_model(model, criterion, optimizer, scheduler, num_epochs, dataloaders, dataset_sizes, device, fold):
    ...
    for epoch in range(1, num_epochs + 1):
        print('\nEpoch: [%d | %d] ' % (epoch, num_epochs))  # epoch + 1
        print('-' * 10)
        ...
        for phase in ['train', 'valid']:
            ...
            bar = Bar('Training: ', max=len(dataloaders[phase]))
            # Iterate over data
            for batch_idx, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    #prec1 = accuracy(outputs.data, labels.data, topk=(1,))[0]  # clw note: 这里计算acc； 如果只有两个类，此时top5会报错;
                    prec1 = accuracy_onehot(outputs.data, labels.data, topk=(1,))[0]
                ...
                # plot progress
                bar.suffix = '({batch}/{size}) | Total: {total:} | ETA: {eta:} | LR: {lr:.6f} | Loss: {loss:.4f} | top1: {top1: .4f} '.format(
                    batch=batch_idx + 1,
...
def run_fold(model, criterion, optimizer, device, fold, num_epochs=10):
    ...
    #     print('lr:', optimizer.param_groups[0]['lr'], 'count:', count)
